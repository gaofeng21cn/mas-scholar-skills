...
当前分层固定为：8 个 active 专业模块由真实可同步 Codex 专业 Skill 支撑，另有 1 个纯机器模块 `mas-scholar-skills.reference-provider-adapters`。它只构造有界 HTTP request 描述并解析外部传入的 provider response；网络、重试、缓存、strict match 和 receipt 都归 OPL Connect。它不是 Skill、Stage owner、引用采纳权威或第 9 个专业 workflow。11 个 aggregate/core Skill 的 hard readiness floor 和 35 个 exported Skill 的物化口径不变。
...
文献工作仍由真实 AI-first 专业 Skill `medical-research-lit` 承接。package adapter profile 覆盖 Crossref、OpenAlex、PubMed eSummary、Europe PMC metadata 加一次有界 full-text probe、Semantic Scholar、Crossmark signal 和 DOI landing metadata；OPL Connect 执行请求并形成 generic strict-match evidence，`medical-research-lit` 负责 query 与筛选策略，MAS 负责 citation acceptance 和 manuscript use。
