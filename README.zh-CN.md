...
MAS 的 stage 主提示词留在 MAS domain-agent 仓：canonical stage source 是 MAS `agent/stages/` 和 `agent/prompts/`。MAS overlay Skill、工作区或 quest 内 `.codex/skills/` 同步副本只是 Codex discovery projection / 兼容面，不是 stage authority 的源头。`write`、`review`、`figure`、`scout` 等阶段负责什么时候进入、证据够不够、交给谁、怎样 route-back、什么算 owner gate。`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design` 和 `medical-research-lit` 是专业技能，负责把已经分配下来的写作、审稿、图件和文献任务做得更专业。
