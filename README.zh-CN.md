...
<p align="center"><strong>MAS 医学论文工作的必需能力包</strong></p>
...
当前分层固定为：8 个 active 专业模块由真实可同步 Codex 专业 Skill 支撑，另有两个纯机器 companion module。`mas-scholar-skills.reference-provider-adapters` 对已知引用执行有界 provider lookup；`mas-scholar-skills.scientific-search-adapters` 通过 Crossref 或 OpenAlex 返回规范化候选列表。两者都只构造 HTTP request 描述并解析外部传入的 response；网络、重试、缓存、适用场景下的 strict match 和 receipt 都归 OPL Connect。它们都不是 Skill、Stage owner、引用采纳权威或专业 workflow。11 个 aggregate/core Skill 的 hard readiness floor 和 35 个 exported Skill 的物化口径不变。
...
文献工作仍由真实 AI-first 专业 Skill `medical-research-lit` 承接。verification profile 覆盖 Crossref、OpenAlex、PubMed eSummary、Europe PMC metadata 加一次有界 full-text probe、Semantic Scholar、Crossmark signal 和 DOI landing metadata；独立 search profile 只覆盖 Crossref/OpenAlex 单步多候选检索。OPL Connect 执行全部请求并形成 generic evidence 或 receipt，`medical-research-lit` 负责 query 与筛选策略，MAS 负责候选采纳、citation acceptance 和 manuscript use。
...
20 个更窄的 specialty specialist，例如 `medical-structural-biology`、
...
named-specialty `SKILL.md` playbook。它们默认物化以保证 Codex 启动前即可
发现，但仍只有明确专科任务或 router 选中时才应使用。
...
使用统一 OPL Packages 入口。安装 MAS 时会解析这个硬依赖，并把全部 35 个
exported Skill 物化到指定 scope：
...
目标目录接收全部 35 个 exported Skill，以及 package 所需的紧凑插件、模块和图库审阅引用。专科 Skill 虽默认存在，仍只在明确匹配的专业任务中路由。不要把本仓整仓、MAS `outputs/display-pack-gallery/`、渲染缓存、单图导出、依赖锁或图库中间产物复制进每个论文工作区或任务目录。
...
单独克隆本仓不会安装 MAS 或 OPL Framework 的可执行面。Provider source 开发时可以检查 OPL Connect 内部 descriptor/materialization primitive，但它们不是第二套用户安装入口。本包不暴露 module execution CLI。
