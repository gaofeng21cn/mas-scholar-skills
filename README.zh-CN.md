...
`MAS Scholar Skills` 是这个仓库和产品的正式名称：一个由 OPL 持有、Codex 可发现、服务 MAS 医学论文能力的外置增强包。历史 `opl-scholarskills` 只保留为兼容别名。本仓是 MAS Scholar Skills 引用、资料包、质量下限、模板、外部学习吸收、模块合同，以及 `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit` 这些可同步专业技能的单源。

MAS 的 stage 主提示词留在 MAS domain-agent 仓：canonical stage source 是 MAS `agent/stages/` 和 `agent/prompts/`。MAS overlay Skill、工作区或 quest 内 `.codex/skills/` 同步副本是 Codex discovery projection / 兼容面，不是 stage authority 的源头；这个同步动作本身必须保留，因为 Codex 依靠 `.codex/skills/` 稳定发现本地技能。`write`、`review`、`figure`、`scout` 等阶段负责什么时候进入、证据够不够、交给谁、怎样 route-back、什么算 owner gate。`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design` 和 `medical-research-lit` 是专业技能，负责把已经分配下来的写作、审稿、图件和文献任务做得更专业。
...
当前分层固定为：10 个能力模块合同，4 个真实可同步专业 Codex Skill，6 个模块合同层能力。`medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit` 是真 Skill；`tables`、`stats`、`omics`、`submit`、`data`、`intake` 目前是 capability module contract / refs / checklist / candidate handoff，不是半成品 Skill，也不是放回 MAS 内部的私有能力。只有当这些模块需要 Codex 主动执行一套稳定专业流程时，才升级成新的真 Skill。
...
当前专业质量地板集中在这四个真 Skill：`medical-figure-design` 必须有 figure contract、evidence chain、candidate loop 和 visual QA；`medical-manuscript-writing` 必须有 argument contract、terminology ledger、paragraph job map、citation integrity 和 data/code availability audit；`medical-manuscript-review` 必须有 shared fact base、reviewer lanes、cross-review synthesis、reviewer action matrix 和 route-back closeout；`medical-research-lit` 必须有 source routing、deduplication、retain/reject/watchlist screening、support-strength grading 和 citation integrity notes。
...
- `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design` 和 `medical-research-lit` 都是本仓真实 Codex Skill，不只是模块 descriptor。
- `tables`、`stats`、`omics`、`submit`、`data` 和 `intake` 是合同层能力：它们统一 vocabulary、ref family、checklist、candidate handoff 和 owner gate，不伪装成已可主动执行的专业 Skill。
- 默认情况下，professional specialist skill 应放在消费它的 domain-agent 仓、贴近 stage 主提示词；只有重型、跨 workspace 复用或需要独立发布/同步时，才拆到外部 pack。本仓就是 MAS 写作、审阅、图件、文献、Display/source refs 的外部 pack 单源。
...
这些模块不是十套独立产品，也不是与 MAS stage 主提示词并列的默认入口。它们是 MAS 可发现和调用的增强能力地图。真正的图表、论文、分析结论、审稿决策和投稿动作，仍由对应的 MAS/domain 系统和负责人确认。
...
专业 Skill 质量地板也吸收 `K-Dense-AI/scientific-agent-skills` 和 `Yuan1z0825/nature-skills` 中可维护的模式：可发现科研技能包、图件契约、先论证后写作、审稿事实基座、来源路由、引用核验、数据可用性检查和审稿回复纪律。这些模式会进入 MAS 持有的专业 Skill，而不是作为第二套 runtime 引入。
...
2. **Professional skill**：默认归 domain repo；本仓只维护可复用外部包里的 `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit` 以及 Display/source refs。这些 skill 可以准备 candidate refs 和专业候选产物，但不能采纳它们。
...
- 本仓维护可分发的 Codex 插件和技能入口、MAS 消费的医学写作/审阅/图件/文献专业 skill、十个能力模块目录、图库人审包和说明文档。
- OPL Framework 维护可执行命令、同步、运行环境桥接、Connect/Fabric 资源能力和工作台动作。
- MAS overlay 仍是 runtime owner 入口。MAS 在本仓外维护 stage 主提示词，并从本仓消费 `medical-manuscript-writing` / `medical-manuscript-review` / `medical-figure-design` 专业 skill。
