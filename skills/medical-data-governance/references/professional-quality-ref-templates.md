...
## Immutable Reviewer Input Snapshot

Fresh manuscript, statistical, reference-integrity, display, and submission
review invocations consume `review_input_snapshot_binding` from MAS and read
only the exact OPL-owned immutable snapshot. The binding shape is:
...
The OPL snapshot manifest is `opl_reviewer_input_snapshot_manifest` schema 1.
Read its generation-owned immutable member locators; never reopen a live
workspace locator after the snapshot is materialized. The ordered binding
inventory must equal the complete reviewed-member projection. Path, locator,
checkout, host, timestamp, reviewer/model identity, model version, skill
version, and runtime version stay outside scientific scope identity.

Missing or stale snapshot evidence produces a refs-only lane route-back
candidate. It cannot create a typed blocker or reject hosted-action liveness.
Malformed exact refs or bytes remain an OPL/MAS validation concern; this pack
does not repair, persist, sign, or reinterpret them.

## Page Hash Evidence Cache Candidate

`medical-display-qc` may produce this refs-only candidate after rasterizing PDF
pages with its fixed contract:
...
version strings from this cache key. The containing MAS display scope may still
change when it binds different raw PDF bytes; a stable pixel hash alone does not
authorize lane reuse.
...
## Epistemic Provenance Context

Professional Skills do not create review signatures or decide review
currentness. Evidence exists to prevent AI hallucination and to make an
analysis, figure, table, citation, or claim auditable, recalculable, and
reproducible in a trusted local workspace.

When OPL or the consuming domain owner supplies an optional
`epistemic_review_scope_ref`, use it only to locate the artifact, claim, and
provenance dependencies relevant to the professional task. The owner surface
is defined by
`opl-framework:contracts/opl-framework/epistemic-review-scope-v2.schema.json`.
Record the specific refs actually inspected in the existing candidate handoff;
do not copy the whole scope, compute a scope digest, build an upstream hash
closure, decide whether a prior receipt is reusable, schedule another review,
or enforce an Attempt budget.

Evidence coverage should be proportional to the task:

- manuscript review: reviewed text and claims, supporting results and
  references, limitations, and the context needed to interpret them;
- statistical review: source data or denominator context, analysis code and
  parameters, results, diagnostics, claims, limitations, and reproduction
  instructions actually assessed;
- reference integrity: cited claims, source records, identifier/status
  evidence, and citation linkage actually checked;
- display QC: visual content, source-result linkage, layout/render context,
  accessibility checks, and the final-scale artifact actually inspected;
- submission prep: package contents, journal requirements, declarations, and
  reopen/export checks, without re-authorizing upstream scientific reviews.

An optional per-artifact SHA-256 may accompany a locator as a stale hint or
deduplication aid. It is not content authority. A hash, path, mtime, checkout,
model, Skill, runtime, checklist, receipt, layout, or package-wrapper change
alone must not invalidate medical, statistical, or reference review. Software
release and installation integrity use a separate OPL contract.

The retired `input_scope_signature_ref` may be read from a legacy handoff only
as a non-authoritative locator. Do not generate it, compare its digest, infer
