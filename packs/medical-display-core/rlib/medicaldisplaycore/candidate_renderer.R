...
build_candidate_layout_override <- function(template_id, display_payload, base_panel_box = NULL, base_guide_box = NULL) {
  ...
  if (identical(template_id, "time_to_event_risk_group_summary")) {
    if (identical(display_payload$plot_variant, "nhanes_decile_grouped_calibration")) {
      panel <- candidate_box("panel_left", "panel", 0.08, 0.20, 0.94, 0.82)
      return(list(
        layout_boxes = list(
          candidate_box("panel_label_A", "panel_label", 0.10, 0.77, 0.13, 0.81),
          candidate_box("panel_left_title", "panel_title", 0.15, 0.86, 0.90, 0.90),
          candidate_box("x_axis_title", "x_axis_title", 0.32, 0.09, 0.68, 0.13),
          candidate_box("y_axis_title", "y_axis_title", 0.01, 0.38, 0.05, 0.64)
        ),
        panel_boxes = list(panel),
        guide_boxes = list(candidate_box("legend", "legend", 0.22, 0.05, 0.78, 0.12)),
        metrics = candidate_metrics_with_renderer(
          template_id,
          display_payload,
          base_panel_box,
          list(
            plot_variant = candidate_non_empty(display_payload$plot_variant, ""),
