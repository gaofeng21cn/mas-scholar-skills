# ggconsort-backed cohort-flow renderer. Dispatched by the pack-level render.R.
...
require_ggconsort <- function() {
  requirement <- "cohort_flow_figure requires prepared R package `ggconsort` >= 0.1.0 with exported `create_consort_data`"
  if (!requireNamespace("ggconsort", quietly = TRUE)) {
    stop(sprintf("%s; renderer does not install packages", requirement))
  }
  installed_version <- utils::packageVersion("ggconsort")
  required_exports <- c(
    "cohort_start",
    "consort_box_add",
    "consort_arrow_add",
  ...
  missing_exports <- setdiff(required_exports, getNamespaceExports("ggconsort"))
  if (
    utils::compareVersion(as.character(installed_version), "0.1.0") < 0 ||
      length(missing_exports) > 0
  ) {
    stop(sprintf(
      "%s; found version %s, missing exports: %s",
      requirement,
      as.character(installed_version),
      if (length(missing_exports) > 0) paste(missing_exports, collapse = ", ") else "none"
    ))
  }
}
...
build_ggconsort_plot <- function(payload) {
  ...
  consort <- ggconsort::cohort_start(data.frame(row_id = 1), "source")
  for (index in seq_along(steps)) {
