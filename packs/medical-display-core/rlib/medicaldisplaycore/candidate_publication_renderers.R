...
candidate_required_collection <- function(payload, field_name, template_id) {
...
candidate_required_text <- function(item, field_name, context) {
...
candidate_required_number <- function(item, field_name, context) {
  value <- item[[field_name]]
  if (is.list(value)) {
...
candidate_required_count <- function(item, field_name, context) {
  value <- candidate_required_number(item, field_name, context)
...
candidate_required_percent <- function(item, field_name, context) {
  value <- candidate_required_number(item, field_name, context)
...
candidate_count_pair <- function(item, numerator_field, denominator_field, context) {
  numerator <- candidate_required_count(item, numerator_field, context)
  denominator <- candidate_required_count(item, denominator_field, context)
  if (denominator < 1) {
...
candidate_compact_number <- function(value) {
...
candidate_plot_dot_range_summary <- function(payload) {
  ...
  rows <- candidate_required_collection(payload, "rows", template_id)
  dot_df <- do.call(rbind, lapply(seq_along(rows), function(index) {
    item <- rows[[index]]
    context <- sprintf("%s.rows[%d]", template_id, index)
    counts <- candidate_count_pair(item, "positive_n", "available_n", context)
    data.frame(
      bmi_category = candidate_required_text(item, "bmi_category", context),
      measure = candidate_required_text(item, "measure", context),
      positive_n = counts[["numerator"]],
      available_n = counts[["denominator"]],
      percent = candidate_required_percent(item, "percent", context),
...
candidate_plot_availability_bar <- function(payload) {
  ...
  availability_df <- do.call(rbind, lapply(seq_along(rows), function(index) {
    item <- rows[[index]]
    context <- sprintf("%s.rows[%d]", template_id, index)
    counts <- candidate_count_pair(item, "available_n", "denominator_n", context)
    data.frame(
      measure = candidate_required_text(item, "measure", context),
      available_n = counts[["numerator"]],
      denominator_n = counts[["denominator"]],
      percent = candidate_required_percent(item, "percent", context),
      stringsAsFactors = FALSE
    )
  }))
...
candidate_plot_adult_multidimensional_phenotype_heatmap <- function(payload) {
  ...
  cells <- candidate_required_collection(payload, "cells", template_id)
  heat_df <- do.call(rbind, lapply(seq_along(cells), function(index) {
    item <- cells[[index]]
    context <- sprintf("%s.cells[%d]", template_id, index)
    data.frame(
      bmi_category = candidate_required_text(item, "bmi_category", context),
      feature = candidate_required_text(item, "feature", context),
      unit = candidate_required_text(item, "unit", context),
      median = candidate_required_number(item, "median", context),
      available_n = candidate_required_count(item, "available_n", context),
      stringsAsFactors = FALSE
    )
  }))
  ...
  heat_df$cell_label <- sprintf(
    "%s\nn=%d",
    vapply(heat_df$median, candidate_compact_number, character(1)),
    heat_df$available_n
  )
...
candidate_plot_xiangya_psychobehavioral_overlap_heatmap <- function(payload) {
  ...
  heat_df <- do.call(rbind, lapply(seq_along(cells), function(index) {
    ...
    context <- sprintf("%s.cells[%d]", template_id, index)
    data.frame(
      phq9_status = candidate_required_text(item, "phq9_status", context),
      gad7_status = candidate_required_text(item, "gad7_status", context),
      count = candidate_required_count(item, "count", context),
      row_percent = candidate_required_percent(item, "row_percent", context),
      stringsAsFactors = FALSE
    )
  }))
  row_sums <- tapply(heat_df$row_percent, heat_df$phq9_status, sum)
  if (any(abs(row_sums - 100) > 0.6)) {
    stop(sprintf("%s row_percent must sum to 100 within each PHQ-9 row", template_id))
  }
...
candidate_plot_adult_bmi_waist_central_adiposity_bar <- function(payload) {
  ...
  bar_df <- do.call(rbind, lapply(seq_along(rows), function(index) {
    item <- rows[[index]]
    context <- sprintf("%s.rows[%d]", template_id, index)
    counts <- candidate_count_pair(item, "central_obesity_n", "available_n", context)
    data.frame(
      bmi_category = candidate_required_text(item, "bmi_category", context),
      percent = candidate_required_percent(item, "central_obesity_percent", context),
      available_n = counts[["denominator"]],
      central_obesity_n = counts[["numerator"]],
      stringsAsFactors = FALSE
    )
  }))
