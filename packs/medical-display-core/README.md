...
Scholar owns the checked-in template descriptors under `templates/*/template.toml`
and their `requirement_profile_ids`. Update these descriptors with their renderer
contracts; the retired MAS manifest bootstrap is not an authoring path.
...
Each template declares `requirement_profile_ids`; consumers pass the union of those IDs
as repeated `opl env run --requirement-profile-id` arguments. The base R profile
contains jsonlite, ggplot2, ggsci, and R's built-in grid. Heatmaps, survival layouts,
baseline tables, t-SNE, UMAP, oncoplots, radar, alluvial, and cohort-flow requests
select their specialist profiles. The pack never installs dependencies itself.
Use `opl env prepare --apply` only after the task's selected profiles are known.

For independent figures in one task, `Rscript render.R --batch batch.json` reuses
one interpreter and shared renderer libraries. The JSON contains `jobs`, each with
unique `case_id`, `template_id`, `request_path`, and optional `render_mode` (default
`final`) and `seed` (otherwise the template controls its analysis seed). A single request retains `--request`, `--template`,
and `--mode`; both paths use the same renderer and state restoration.
Output paths must be distinct between jobs. Request/output paths resolve from the
working directory. Each job restores options, environment variables, working
directory, random seed, graphics devices, search path, and renderer bindings.
Ordinary rendering errors are reported per job while independent jobs continue;
a state-restoration failure aborts the remaining jobs. Any failed job returns a
nonzero process status.

Each completed job writes `<request_path>.execution.json` with its result and
`OPL_ENV_EXECUTION_ID` / `OPL_ENV_MANIFEST_REF` binding. The batch writes
`<batch_path>.results.json` (or explicit `result_path`) atomically after every job,
so cancellation retains completed progress. These records report execution only,
not scientific or publication acceptance. A batch uses one already prepared
environment; it is not a persistent session across tasks.

Run `python3 packs/medical-display-core/tests/benchmark_render_batch.py` from the
repository root for three ten-figure single/batch comparisons, exact PNG and layout
comparison, error isolation, output collision, cancellation, and execution-binding
checks. Results and figures are written outside the source tree. Batch defaults
must be enabled by consumers only after this benchmark meets the 20% reduction
threshold on the target runtime.
...
create `Rplots.pdf`. The cohort regression requires the prepared mature ggconsort API
(`>= 0.1.0` with `create_consort_data`). Without that dependency the default gate reports
`dependency_unavailable` while retaining the five registry renderer checks; currentness or
renderer-readiness evidence must run the same regression in the prepared display environment.
