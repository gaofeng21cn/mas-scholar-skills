...
本次 full force-render 资产准备状态：`cleaned_for_force_render`；46 个可视模板全部重新渲染，`cache_miss=46`。真正的论文渲染 freshness 仍由 paper-local render cache key、layout sidecar、dependency run-context 和真实 artifact 审计证明。
