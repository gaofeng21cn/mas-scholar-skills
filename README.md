...
The current classification is fixed: eight active professional modules backed by syncable real Codex specialist skills, plus `mas-scholar-skills.reference-provider-adapters`, a machine-only read adapter module. The adapter builds bounded HTTP request descriptions and parses supplied provider responses; OPL Connect owns network execution, retry, cache, strict matching, and receipts. The adapter is not a Skill, Stage owner, citation authority, or ninth professional workflow. Historical `opl.scholarskills.*` ids remain legacy aliases/provenance only. The 11 aggregate/core Skills still define the hard readiness floor, and every active MAS workspace or quest receives all 35 exported skills for native Codex discovery.

For literature work, `medical-research-lit` remains the AI-first specialist skill. The package adapter profile covers Crossref, OpenAlex, PubMed eSummary, Europe PMC metadata plus one bounded full-text probe, Semantic Scholar, Crossmark signals, and DOI landing metadata. OPL Connect executes those requests and produces generic strict-match evidence; `medical-research-lit` owns query and screening strategy, while MAS owns citation acceptance and manuscript use.
