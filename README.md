...
The current classification is fixed: eight active professional modules backed by syncable real Codex specialist skills, plus two machine-only companion modules. `mas-scholar-skills.reference-provider-adapters` verifies a known reference through bounded provider lookups; `mas-scholar-skills.scientific-search-adapters` searches Crossref or OpenAlex and returns normalized candidate lists. Both modules only build HTTP request descriptions and parse supplied responses. OPL Connect owns network execution, retry, cache, strict matching where applicable, and receipts. Neither module is a Skill, Stage owner, citation authority, or professional workflow. Historical `opl.scholarskills.*` ids remain legacy aliases/provenance only. The 11 aggregate/core Skills still define the hard readiness floor, and every active MAS workspace or quest receives all 35 exported skills for native Codex discovery.

For literature work, `medical-research-lit` remains the AI-first specialist skill. The verification profile covers Crossref, OpenAlex, PubMed eSummary, Europe PMC metadata plus one bounded full-text probe, Semantic Scholar, Crossmark signals, and DOI landing metadata. The separate search profile covers Crossref and OpenAlex multi-candidate discovery in one HTTP step. OPL Connect executes all requests and produces generic evidence or receipts; `medical-research-lit` owns query and screening strategy, while MAS owns candidate acceptance, citation acceptance, and manuscript use.
