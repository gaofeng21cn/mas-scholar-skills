...
Machine boundary: 本文是人读导航。本 repo 的 module catalog、validator policy、candidate artifact engine rules、Codex 暴露策略与 Skill pack 真相以 `contracts/scholar-skills-capability-modules.json`、`.codex-plugin/plugin.json`、各 `skills/medical-*/SKILL.md` 与 gallery manifest 为准；`contracts/scholar-skills-opl-consumption-projection.json` 是从 canonical contract 确定性生成的 OPL consumer projection，不是第二真相源。`opl-scholarskills` 只作为 history/tombstone/provenance 名称，不再有 active `SKILL.md`。可执行 `opl scholar-skills *` CLI、通用校验/materialization 与 Connect 同步行为仍以 OPL Framework readback 为准。

## 基本定位

`MAS Scholar Skills` 是 `mas-scholar-skills` 仓的正式产品名：服务 MAS 医学论文能力的 OPL-owned 外置增强包。历史 `opl-scholarskills` 仅作为 legacy alias 和 provenance。它不是通用 OPL runtime 基座，不是第十一个 OPL 品牌模块，也不是 MAS domain truth owner。它给 MAS overlay 和 MAS stage operating prompts 提供可发现、可校验、可接入 OPL runtime env 的能力模块说明、source pack、quality floor、template、candidate ref 和外部学习吸收材料；同时维护 `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-figure-style`、`medical-figure-composer`、`medical-research-lit`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep`、`medical-data-governance` 这些专业 Codex skill，供 MAS 同步消费。
...
历史 `opl.scholarskills.*` module id 只保留为 legacy alias/provenance；新合同、Skill guidance 和 CLI 示例默认使用 `mas-scholar-skills.*`。

MAS Scholar Skills 正式承接八个 active 专业模块的 MAS 增强材料 source / contract / docs：`lit`、`tables`、`stats`、`submit`、`write`、`review`、`data` 与 `display` 同级进入本仓 contract snapshot、Skill 入口和人读说明。这个承接只限 refs-only / no-authority handoff；公共边界见 `docs/no-authority-boundary.md`，机器路由和 false-authority flags 见 `contracts/capability_map.json`。所有 active module 都必须能说明 `source_pack_ref`、`candidate_package_ref`、`execution_receipt_ref` 和 `owner_gate_handoff_ref`，这些 ref 只描述候选输入、候选包、未签执行回执和下游 owner gate 交接点。

当前采用 `AI First, Contract Light`：claim/evidence 判断、negative finding、
quality review、route-back 选择、figure/table/source QA、引用支持和 FAIR
数据治理判断，默认落在既有 `medical-*` professional Skill 正文中。Module
contract 只保留 module id、索引、ref family、receipt shape、no-authority
...
`verdict_candidate`、`stop_or_continue_recommendation`、`route_back_candidate`
...
20 个 optional advanced 和 medical-method specialist 名称继续保留为真实
`SKILL.md`，例如 `medical-structural-biology`、
`medical-protocol-and-sap-planner`、`medical-reference-integrity-auditor`、
`medical-display-qc`、`medical-research-portfolio-memory-curator` 和
...
4 个原本独立的 optional professional skill 退役为 mode，不再保留
`SKILL.md` metadata：`medical-evidence-gap-triage-reviewer` 由
`medical-evidence-integrity-reviewer` 的 evidence-gap triage mode 覆盖；
`medical-methodology-routeback-reviewer` 和
`medical-owner-gate-handoff-reviewer` 由
`medical-publication-routeback-reviewer` 的 methodology routeback / owner-gate
handoff modes 覆盖；`medical-publication-strategy-memory-curator` 由
`medical-research-portfolio-memory-curator` 的 publication strategy memory
mode 覆盖。退役目录只保留 `TOMBSTONE.md` redirect，contract 记录
`capability_preserved=true` 和 `default_exposure=false`。
...
| 可选专科 Skill | 4 个 router/reviewer + 20 个 named-specialty specialists；4 个 retired mode redirect | 只在 named specialty task 中发现和同步真实 Skill；退役能力通过 reviewer mode 覆盖，不作为 active metadata 暴露。 |
...
分层治理。弹性专业判断保留在 Skill 层：AI 可以在 skill 内给出
`verdict_candidate`、`route_back_candidate`、`stop_or_continue_recommendation`
...
当前 helper 覆盖是刻意选择，不是“把所有 Skill 脚本化”：35 个 repo 内 `SKILL.md` 中，默认 install 的主路径是 aggregate + core 10 个专业 skill；optional named-specialty install 包括 4 个 router/reviewer 和 20 个 named-specialty skill。28 个专业、方法或专科目录仍带 `kernel.py` 作为历史低成本确定性 helper；`mas-scholar-skills` 是 aggregate 入口，`medical-figure-design` 是 Display 编排入口，`medical-methodology-planner`、`medical-evidence-integrity-reviewer`、`medical-publication-routeback-reviewer` 与 `medical-advanced-biomed-router` 是 optional router/reviewer 入口。4 个 retired optional professional skill 只保留 `TOMBSTONE.md` redirect，不参与 active discovery。`opl-scholarskills` 只保留为 tombstone/provenance alias，不参与 active discovery。
...
本轮继续吸收 `K-Dense-AI/scientific-agent-skills`、`Yuan1z0825/nature-skills`、Co-Scientist JIT affordance 与 OpenScience local-first 模式的可迁移经验，但落点是真实 Skill 的 AI-first 工作流，不新增并列 stage authority，也不要求先安装外部 runtime。

| Skill | 新增质量要求 |
| --- | --- |
| `medical-figure-design` | figure contract、core conclusion、evidence chain、figure archetype、renderer decision、schematic / infographic evidence boundary、style brief、candidate set、critic review as repair hint、claimType / graphWarnings claim-drift check、annotation-to-source-regeneration repair hint、rerun/render receipt consumption、triggered meta-review、final-scale visual QA、style/composition subskill routing 和 reviewer packet。 |
| `medical-figure-style` | style-only visual grammar QA、data fidelity、claim-title truth、excluded-row handling、comparability、label economy、color-vision robustness、final-scale visual QA、critique-as-repair hint、annotation-to-source-regeneration repair hint、rerun/export receipt consumption 和 export-visible style route-back。 |
| `medical-figure-composer` | compose-only multi-panel outline、existing panel render receipts、panel-letter/gutter/resized-text checks、crop-level consistency、composite review as repair hint、final-scale visual QA、annotation-to-source-regeneration route-back、rerun/composite receipt consumption 和 export QA。 |
| `medical-manuscript-writing` | one-sentence argument、paper-mission framing、claim/evidence strength calibration、terminology ledger、paragraph job map、section contract、citation integrity、claimType / graphWarnings prose warning、critique-as-repair hint、reusable lesson extraction、opportunistic source/journal/table/figure prefetch、rerun receipt consumption、figure/table binding、data/code availability audit。 |
| `medical-manuscript-review` | review fact base、paper-mission framing review、claim/evidence strength review、technical/significance/reader/validity/scholar-evaluation reviewer lanes、critique-as-repair hint、reusable lesson extraction、triggered meta-review、opportunistic knowledge prefetch、claimType / graphWarnings claim-warning check、annotation-to-source-regeneration repair hint、cross-review synthesis、reviewer action matrix、citation repair、revision delta audit、rerun receipt consumption、route-back closeout。 |
| `medical-research-lit` | PubMed/PMC-first source routing through `opl connect scientific search --provider pubmed`、OPL Connect scientific connector refs consumption、PubMed compatibility entry awareness、Crossref/OpenAlex metadata / coverage / citation graph fallback refs only、opportunistic citation/source prefetch、claimType / graphWarnings source-support warning、annotation-to-source-regeneration repair hint、source-screening matrix、query plan、fallback reason、deduplication、retain/reject/watchlist screening、source verification、support-strength matrix、claim-support map、citation integrity floor、rerun/query receipt consumption。 |
| `medical-statistical-review` | statistical question、estimand/target parameter、analysis plan fit、EDA profile、model specification refs、denominator/missingness、assumption diagnostics、effect size/uncertainty、multiplicity/sensitivity、claimType / graphWarnings statistical-claim warning、annotation-to-source-regeneration repair hint、critique-as-repair hint、triggered meta-review、rerun analysis receipt consumption、statistical action matrix。 |
| `medical-table-design` | table job、table shell、source metric、denominator、statistical display、table QC、claim-table alignment、claimType / graphWarnings table-claim warning、annotation-to-source-regeneration repair hint、critique-as-repair hint、rerun/table receipt consumption、journal table contract。 |
| `medical-submission-prep` | journal instruction、reporting guideline、declaration inventory、data/code availability、package consistency、claimType / graphWarnings package-warning check、annotation-to-source-regeneration repair hint、opportunistic journal/source/declaration prefetch、reviewer response candidate、author-input fields、rerun/export receipt consumption、submission action matrix。 |
| `medical-data-governance` | data asset manifest、dataset manifest、data dictionary/codebook、cleaning/normalization readiness、source lineage、semantic reproducible capsule、version-diff impact、annotation-to-source-regeneration repair hint、project-local ledger pointer/hash、claimType / graphWarnings source-warning check、critique-as-repair hint、triggered meta-review、rerun data-operation receipt consumption、study binding、privacy/access tier、lifecycle/retention guardrail、owner-gate handoff。 |

这些新增 affordance 都是现有 Skill 的质量地板，不是第九个 module、不是 optional specialty skill、不是 runtime worker，也不是 MAS authority surface。各 skill 的新增 judgment refs 是消费提示，不是新 module：Write 负责
claim/evidence 与 negative/equivocal finding 的写作判断；Review 负责质量
审阅、route-back 和 stop/continue；Lit 负责 source-support 与 citation
integrity；Stats 负责 estimand、negative/equivocal result 和 claim-strength；
Tables 负责 table-vs-figure、denominator 和 claim-table alignment；Submit 负责
author/institutional gap、package consistency 和 stop/continue；Data 负责
source lineage、FAIR、privacy/access 与 lifecycle；Display 负责 visual QA、
figure/table display choice 和 hard-evidence route-back。
...
本文件只维护 module catalog、ref family、quality floor 与 candidate handoff
说明。Stage / specialist / connector / contract 的 owner split 统一见
`docs/mas-scholar-skills-operating-model.md`；owner receipt、typed blocker、
publication readiness 等 false-authority 边界统一见
`docs/no-authority-boundary.md` 和 `contracts/capability_map.json`。
...
`medical-display-core` 现在同时声明 generic OPL Pack descriptor：`packs/medical-display-core/opl_pack.json`。OPL 基座通过 `opl pack inspect/check/run/gallery --pack <path>` 消费这类资源；Display 不拥有 OPL 顶层 `opl display` 命令。Pack-level `default_execution_mode=declared_by_template` 只表示执行方式由各 `templates/*/template.toml` 声明，不代表证据图以 Python 为主路径。R/ggplot2 模板统一走 pack 根 `render.R`，`render_mode=final|candidate` 是请求字段，不再用每模板 `render_candidate.R` 维护第二入口。

Display pack 库存一致性由 `scripts/verify-display-gallery-pack.py --sync-opl-pack` / `--check` 从 `canonical_template_catalog.json`、模板 descriptor 和 `opl_pack.json` 派生维护：`--sync-opl-pack` 只重写 `opl_pack.json` 中 `role=template` 的 resources，其他 resources 保持原顺序和内容；`--check` fail-fast 校验当前 descriptor 与生成结果一致。OPL template resources 必须与 catalog template 集合一致，R/ggplot2 模板的 `supported_render_modes` 必须与 pack-level `final|candidate` 声明一致。

医学 SCI 初稿质量 floor 分布在多个模块：Lit 负责 `reference_integrity_floor_ref`，用于提示缺失参考文献、占位 citation 和 claim/source 覆盖缺口；Write 负责 `manuscript_body_volume_floor_ref` 与 `internal_report_prose_route_back_ref`，用于提示正文体量不足和内部报告式工作流文风；Display 负责 `figure_table_volume_and_clinical_value_ref` 与 `figure_polish_alignment_ref`，用于提示结果图表体量、临床解释价值和新版 figure-polish 技能一致性；Review 汇总为 `scholarskills_medical_sci_initial_draft_quality_floor.v1`，并加入 `registry_descriptive_scientific_boundary_ref`、`submission_declaration_completeness_ref`、`registry_data_lock_enrollment_boundary_ref`、`diagnostic_provenance_caveat_ref`、`figure_caption_content_consistency_ref`、`supplementary_missingness_atlas_ref`、`adult_bmi_sensitivity_ref`、`pdf_nonblank_figure_export_qc_ref`、`journal_figure_numbering_ref`、`wide_table_routing_ref` 和 `discussion_theme_compression_ref`，用于提示 descriptive registry / atlas 初稿不要把可用性、缺失性、诊断字段阳性或描述性分布写成因果、预测、患病率/负担或 publication-ready 结论，也不要遗漏伦理/知情同意/数据可得性/基金/COI、把 visit-date coverage 写成 enrollment/data-lock、让图注提到未呈现领域，只口头声称 missingness atlas 而不物化补充表，在有儿童/年龄缺失记录时缺少成人已知年龄 BMI 敏感性表，PDF 图区空白，图号重复，主文宽表不可读，或 Discussion 未压缩成注册数据结构、成人代谢表型和亚队列临床深度主题。这些 ref 只能生成 `verdict_candidate`、`route_back_candidate` 和 `stop_or_continue_recommendation`，不能声明 MAS publication ready、owner accepted、typed blocker、current_package authority、quality verdict 或 reviewer receipt。
...
FeedbackOps adapter 同样保持 refs-only：`feedbackops_refs_only_adapter_policy` 把 MAS Scholar Skills 声明为 OPL FeedbackOps 的 capability adapter，profile 为 `target_agent_feedback_external_suite`。它可以从 `delivery_feedback_ref` / `feedbackops_intake_ref` 生成 `candidate_refs`、quality hints、Display/Write/Review capability suggestions、`route_back_candidate_ref` 和 `stop_or_continue_recommendation_ref`，供 MAS 或 OMA 作为 evidence input 消费。`feedback_self_evolution_trigger` 则把这条链显式绑定到 OPL 标准 self-evolution trigger，说明这是 `framework_capability_package` 形态下的 machine-readable handoff surface。MAS/OMA 消费后若要签 `owner_receipt_ref`、产出 typed blocker、质量 verdict、artifact mutation 或 current-package update，必须回到各自 owner surface；MAS Scholar Skills 不能写 MAS/current_package，也不能声明 quality verdict、owner acceptance、typed blocker 或 publication readiness。

当 `materialize --emit-candidate-artifacts` 显式启用时，OPL 的通用 materializer 消费本仓 projection 中的八个 declarative candidate artifact engine spec，为每个 active module 写出专业候选体、`input_requirements`、`validation_checks`、`engine_receipt_ref` 和 sha256 refs。Display 输出 SVG visual-plan candidate；Write/Review/Submit 输出 Markdown candidate；Tables/Stats/Lit/Data 输出 JSON structured candidate。医学 module id、validator 与 engine rule 归本仓；OPL 只持有通用校验、materialization、安装/同步与 provenance。输出仍保持 `counts_as_paper_truth=false`、`counts_as_owner_receipt=false`、`can_authorize_publication_readiness=false`、`can_claim_quality_verdict=false`、`can_claim_artifact_authority=false`，不能替代 domain owner consumption。

## OPL 消费投影

`scripts/export-opl-consumption-projection.py` 从 canonical module contract 生成 `contracts/scholar-skills-opl-consumption-projection.json`。投影固定八模块顺序、validator policy、candidate artifact engine spec、source fingerprint 与全 false authority；`scripts/verify.sh` 会运行 exporter `--check`，任何手工漂移或 source/projection 不一致都会失败。OPL Framework 只能消费该投影并实现 generic validator/catalog/materializer，不能再复制医学 module catalog、医学 validator 分支或八套 engine rule。

仓内提供 repo-tracked Codex plugin surface：`.codex-plugin/plugin.json`、`skills/mas-scholar-skills/SKILL.md`，以及真实医学论文专业 Skill `skills/medical-manuscript-writing/SKILL.md`、`skills/medical-manuscript-review/SKILL.md`、`skills/medical-figure-design/SKILL.md`、`skills/medical-figure-style/SKILL.md`、`skills/medical-figure-composer/SKILL.md`、`skills/medical-research-lit/SKILL.md`、`skills/medical-statistical-review/SKILL.md`、`skills/medical-table-design/SKILL.md`、`skills/medical-submission-prep/SKILL.md`、`skills/medical-data-governance/SKILL.md`。本仓是 MAS Scholar Skills skill pack、module catalog snapshot、医学论文专业 skill 和 gallery review refs 的 source of truth；该 skill pack 把 canonical contract snapshot、CLI readback 入口、AI-first specialist-skill workflow 和 no-authority guard 暴露给 Codex discovery / sync layer。默认 workspace / quest 暴露由 `codex_skill_exposure_policy` 限定为 aggregate `mas-scholar-skills` 加 core medical-paper skills；optional specialist skills 只在 named specialty scope 同步。`opl-scholarskills` 是 tombstone/provenance alias，不再有 active `SKILL.md`。它不能替代 OPL Framework 中的 executable `opl scholar-skills *` CLI、MAS runtime owner surface、domain owner receipt、typed blocker、runtime evidence 或 paper artifact authority。
...
该目录是 OPL-managed local discovery copy，只承载消费方会话需要发现的 aggregate/core `SKILL.md`、plugin/module refs、紧凑 gallery review refs 和轻量 manifest。可选专科 Skill 只在明确 named specialty 任务中同步；`opl-scholarskills` 不同步为 active skill。它不是 MAS domain truth、不是 MAS owner receipt、不是 typed blocker、不是 runtime queue，也不是 publication/package authority。不要把本 repo 整仓、MAS 渲染输出、gallery 中间产物或 OPL/MAS 程序源码复制到论文目录或 quest。
...
只有显式 `--scope codex` 才写用户 Codex plugin registry / config。旧的 MAS program-repo mirror 只能作为历史迁移或显式开发 surface，不是推荐的 runtime quest discovery surface。App/workbench 若暴露 sync action，应路由到同一 workspace/quest-local install model，并支持 dry-run readback。历史 `--domain mas-scholar-skills` 只用于既有工作区兼容；新同步使用 `--domain mas-scholar-skills`。
...
MAS 可以把八个 active `mas-scholar-skills.*` 模块作为 `current_owner_delta` 的 refs-only capability request，桥接到 MAS Scientific Capability Registry 的 candidate artifact refs。历史 `opl.scholarskills.*` id 只作为 legacy alias/provenance。Display 仍可复用 MAS Display Pack 的 candidate artifact refs；Tables、Stats、Lit、Write、Review、Submit 和 Data 则通过同一 OPL module descriptor / execution receipt 形状暴露专业候选 refs。但 MAS 仍持有 study truth、publication truth、quality verdict、artifact authority、owner receipt、typed blocker、human gate 和当前 package authority。
...
`mas-scholar-skills.display` 的人审 gallery 发布包放在本 repo 的 `gallery/medical-display/`，方便安装、运维和审阅时直接打开。该发布包复制自 MAS Display Pack 的最终 docs/gallery surface，只保留 PDF、manifest、reference、status、quality audit 和 snapshot，不复制 MAS 渲染中间目录。
...
这些 refs 证明有人可审的 Display gallery surface 存在，也可作为代表模板 reference-snapshot golden backing，用于对账 compact review package、manifest refs、descriptor 口径和 included-file hash；它们不证明 live renderer 当前可运行、pixel/layout regression 已覆盖、visual parity 完成、publication-ready、owner accepted 或 MAS paper artifact ready。当前 MAS gallery truth 仍需按 MAS Display Pack owner gate、fresh rendered artifact inspection 和 paper-local visual audit 判定，MAS Scholar Skills 只能把它作为 `visual_audit_or_gallery_preview_ref` 的下游人审入口。
