...
`MAS Scholar Skills` 是 `mas-scholar-skills` 仓的正式产品名：服务 MAS 医学论文能力的 OPL-owned 外置增强包。历史 `opl-scholarskills` 仅作为 legacy alias 和 provenance。它不是通用 OPL runtime 基座，不是第十一个 OPL 品牌模块，也不是 MAS owner skill。它给 MAS overlay 和 MAS `medical-research-*` 技能提供可发现、可校验、可接入 OPL runtime env 的能力模块说明、source pack、quality floor、template、candidate ref 和外部学习吸收材料。
...
MAS overlay skill 是医学论文工作的主入口。写作、审阅和图件三类高频问题默认走 MAS-owned `medical-research-write`、`medical-research-review` 和 `medical-research-figure`。本仓不规划 `opl-scholar-write`、`opl-scholar-review` 或 `opl-scholar-display` 作为与这些 MAS skill 并列的默认入口。

当 MAS skill 需要能力增强时，才调用 MAS Scholar Skills 读取模块说明、source pack、quality floor、template、candidate refs 或 route-back hints。如果写作、审阅或图件质量不足，修复方向是优化 MAS medical-research skill，并在本仓同步 refs、pack 或 quality floor；不是在本仓新增第二套 MAS owner skill。
...
仓内提供 repo-tracked Codex plugin surface：`.codex-plugin/plugin.json`、`skills/mas-scholar-skills/SKILL.md` 和真实专家 Skill `skills/medical-research-lit/SKILL.md`。本仓是 MAS Scholar Skills skill pack、module catalog snapshot 和 gallery review refs 的 source of truth；该 skill pack 把 canonical contract snapshot、CLI readback 入口和 no-authority guard 暴露给 Codex discovery / sync layer。`skills/opl-scholarskills/SKILL.md` 只保留 legacy alias。它不能替代 OPL Framework 中的 executable `opl scholar-skills *` CLI、domain owner receipt、typed blocker、runtime evidence 或 paper artifact authority。
