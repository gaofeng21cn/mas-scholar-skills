...
`medical-display-core` 现在同时声明 generic OPL Pack descriptor：`packs/medical-display-core/opl_pack.json`。OPL 基座通过 `opl pack inspect/check/run/gallery --pack <path>` 消费这类资源；Display 不拥有 OPL 顶层 `opl display` 命令。R/ggplot2 模板统一走 pack 根 `render.R`，`render_mode=final|candidate` 是请求字段，不再用每模板 `render_candidate.R` 维护第二入口。

Display pack 库存一致性由 `scripts/verify-display-gallery-pack.py --check` 从 `canonical_template_catalog.json`、模板 descriptor 和 `opl_pack.json` 派生校验：OPL template resources 必须与 catalog template 集合一致，R/ggplot2 模板的 `supported_render_modes` 必须与 pack-level `final|candidate` 声明一致。
