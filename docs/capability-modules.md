...
`MAS Scholar Skills` 是 `opl-scholarskills` 仓当前产品阶段的定位：服务 MAS 医学论文能力的 OPL-owned 外置增强包。它不是通用 OPL runtime 基座，不是第十一个 OPL 品牌模块，也不是 MAS owner skill。它给 MAS overlay 和 MAS `medical-research-*` 技能提供一组可发现、可校验、可接入 OPL runtime env 的 `capability module` descriptor、source pack、quality floor、template、candidate ref 和外部学习吸收材料。
...
ScholarSkills 不改 `BrandModuleId` 枚举；它作为这些品牌模块共同管理、但面向 MAS 消费的增强包存在。
...
ScholarSkills 正式承接全部十个模块的 MAS 增强材料 source / contract / docs：`lit`、`tables`、`stats`、`submit`、`write`、`review`、`omics`、`data`、`intake` 与 `display` 同级进入本仓 contract snapshot、Skill 入口和人读说明。这个承接只限 refs-only / no-authority handoff。所有模块都必须能说明 `source_pack_ref`、`candidate_package_ref`、`execution_receipt_ref` 和 `owner_gate_handoff_ref`，但这些 ref 只描述候选输入、候选包、未签执行回执和下游 owner gate 交接点，不写 MAS domain truth、owner receipt、typed blocker、human gate、publication readiness authority、runtime queue 或 provider attempt。
...
当 MAS skill 需要能力增强时，才调用 ScholarSkills 读取 module descriptor、source pack、quality floor、template、candidate refs 或 route-back hints。如果写作、审阅或图件质量不足，修复方向是优化 MAS medical-research skill，并在本仓同步 refs、pack 或 quality floor；不是在本仓新增第二套 MAS owner skill。
