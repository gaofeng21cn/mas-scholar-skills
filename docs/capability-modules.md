...
可选 optional advanced specialist pack 额外提供真实 Codex Skill：
...
`scientific-compute-runner`。它们吸收 AcademicForge HEAD
`54a2f333973147a1fd703caea6f12252e1f227d6` 的专业 playbook，但只作为
refs-only / no-authority 外置专科 helper，不新增 active core module，不替代
默认医学论文专业 Skill，缺失时不阻断 MAS ordinary progress。科学计算相关
substrate 仍由 OPL Runway / Connect / Fabric 或消费方 compute owner 持有；
Skill 只持有 AI diagnostic playbook 和 deterministic receipt refs。

可选 medical-method specialist pack 也额外提供真实 Codex Skill：
