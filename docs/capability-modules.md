...
当前固定采用三类物理形态，避免把能力目录误读成“十个假 Skill”：

| 层级 | 当前对象 | 作用 |
| --- | --- | --- |
| 能力模块合同 | `display`、`tables`、`stats`、`omics`、`lit`、`write`、`review`、`submit`、`data`、`intake` | 统一 module id、vocabulary、ref family、checklist、candidate handoff、receipt shape 和 owner gate。 |
| 真实专业 Skill | `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit`、`medical-statistical-review`、`medical-table-design`、`medical-submission-prep` | 给 Codex 执行稳定医学论文专业流程的 AI-first playbook，并通过 OPL Connect 同步到 workspace / quest `.codex/skills/`。 |
| 合同层模块 | `omics`、`data`、`intake` | 先提供 refs、checklist、candidate package、quality floor 和 owner-gate handoff；当需要主动执行稳定专业流程时，再升级成真 Skill。 |

这三个合同层模块不是从 MAS 拆出的旧真 Skill，也不是回流到 MAS 内部的私有实现。它们来自 MAS 医学论文链条的高频能力需求、OPL capability registry 的可发现/可回执要求，以及外部学习吸收后的 refs/checklist 形状。它们的必要性是作为机器合同层：让 MAS、OPL Framework、OPL Connect/Fabric 和专业 Skill 用同一套能力名、引用族和 owner gate 交接，而不是把一次性提示词、脚本路径或临时清单散落到各处。

## 专业 Skill 质量地板

本轮继续吸收 `K-Dense-AI/scientific-agent-skills` 与 `Yuan1z0825/nature-skills` 的可迁移经验，但落点是七个真 Skill 的 AI-first 工作流，不新增并列 stage authority，也不要求先安装外部 runtime。
