...
当前 helper 覆盖是刻意选择，不是“把所有 Skill 脚本化”：34 个 Skill 中 29 个专业或方法 Skill 带 `kernel.py`；`mas-scholar-skills`、`opl-scholarskills` 是 aggregate/bridge 入口，`medical-figure-design` 是 Display 编排入口，`medical-publication-strategy-memory-curator` 与 `medical-evidence-gap-triage-reviewer` 是 refs-only AI review/triage playbook，继续由已有 helper 承接确定性小工具。
