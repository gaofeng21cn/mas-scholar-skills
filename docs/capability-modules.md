...
| 真实专业 Skill | `medical-manuscript-writing`、`medical-manuscript-review`、`medical-figure-design`、`medical-research-lit` | 给 Codex 执行稳定医学论文专业流程的 AI-first playbook，并通过 OPL Connect 同步到 workspace / quest `.codex/skills/`。 |
| 合同层模块 | `tables`、`stats`、`omics`、`submit`、`data`、`intake` | 先提供 refs、checklist、candidate package、quality floor 和 owner-gate handoff；当需要主动执行稳定专业流程时，再升级成真 Skill。 |
