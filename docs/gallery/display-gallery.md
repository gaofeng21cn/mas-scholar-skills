...
当前 snapshot 对应 `54` 个 canonical templates：`43` 个 R/ggplot2 evidence figures、`1` 个 R/ggplot2 reporting-flow figure、`1` 个 Python design figure 和 `9` 个 non-visual table shells。Gallery 展示 `46` 个 visual templates，其中只对 `table1_baseline_characteristics` 提供 table preview。精确计数和成员始终以 manifest/snapshot 与 repo-native verifier 为准。
